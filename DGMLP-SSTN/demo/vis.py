...
# 添加新函数：从train_images.txt中查找目标图像的索引
def find_image_index(images_file, target_image):
    with open(images_file, 'r') as f:
        images = [line.strip() for line in f]
    
    if target_image in images:
        return images.index(target_image)
    else:
        print(f"目标图像 {target_image} 未在文件列表中找到")
        return -1

# 添加新函数：获取指定索引的GT关键点坐标
def get_gt_keypoints_from_h5(h5_file, index):
    with h5py.File(h5_file, 'r') as f:
        # 获取关键点数据
        if 'part' in f:
            keypoints_data = f['part'][:]
            
            # 获取指定索引的关键点
            if 0 <= index < keypoints_data.shape[0]:
                gt_keypoints = keypoints_data[index]
                return gt_keypoints
            else:
                print(f"索引 {index} 超出范围")
                return None
        else:
            print("H5文件中没有'part'键")
            return None

# 修改get_pose2D函数以支持从train_images.txt和train.h5加载关键点
def get_pose2D(video_path, output_dir, args):
    if args.keypoints == 'gt':
        # 使用真实关键点数据
        print('Using ground truth keypoints from train.h5...')
        
        # 获取视频文件名
        video_name = os.path.basename(video_path)
        
        # 定义train_images.txt和train.h5的路径
        images_file = r'e:\pycharm文件\GraphMLP-sstn-超图\GraphMLP-main\demo\video\train_images.txt'
        h5_file = r'e:\pycharm文件\GraphMLP-sstn-超图\GraphMLP-main\demo\video\train.h5'
        
        # 从train_images.txt中查找目标图像的索引
        index = find_image_index(images_file, video_name)
        
        if index != -1:
            print(f"目标图像 '{video_name}' 在文件列表中的索引为: {index}")
            
            # 获取对应索引的GT关键点坐标
            gt_keypoints = get_gt_keypoints_from_h5(h5_file, index)
            
            if gt_keypoints is not None:
                # 转换关键点格式以适应后续处理
                # 格式应该是(1, 1, 17, 2)，其中1是视频数量，1是帧数，17是关节点数
                keypoints = np.expand_dims(np.expand_dims(gt_keypoints, axis=0), axis=0)
                print('成功加载GT关键点数据！')
            else:
                # 如果无法加载真实关键点，创建占位数据
                print('Warning: 无法加载真实关键点数据，使用占位数据')
                keypoints = np.zeros((1, 1, 17, 2))
        else:
            # 如果找不到图像，创建占位数据
            print('Warning: 找不到图像，使用占位数据')
            keypoints = np.zeros((1, 1, 17, 2))
    else:
        # 原始逻辑：使用HRNet生成2D姿态
...
def get_pose3D(video_path, output_dir, fix_z):
    ...
    args.layers, args.channel, args.d_hid, args.token_dim, args.frames = 3, 512, 1024, 256, 1
    args.pad = (args.frames - 1) // 2
    args.previous_dir = 'checkpoint/pretrained/sstn_dy'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--video', type=str, default='S1_Directions.54138969_000161.jpg', help='input video')
    parser.add_argument('--gpu', type=str, default='0', help='input video')
    parser.add_argument('--fix_z', action='store_true', help='fix z axis')
    parser.add_argument('--keypoints', type=str, default=None, choices=['gt'], help='use ground truth keypoints')
    ...
    get_pose2D(video_path, output_dir, args)
