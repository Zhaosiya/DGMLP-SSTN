...
from model.DGMLPSSTN import Model
    ...
    for i, data in enumerate(tqdm(dataloader, 0)):
        ...
        for module in model.modules():
            if hasattr(module, 'regularization_loss'):
                sstn_loss += module.regularization_loss()

        total_loss = loss + 0.01 * sstn_loss  # 正则化系数
...
def test(actions, dataloader, model, model_refine):
    ...
    # 新增：返回详细的action_error用于记录每个动作的误差
    return p1, p2, pck, auc, action_error
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        ## test
        with torch.no_grad():
            p1, p2, pck, auc, action_error = test(actions, test_dataloader, model, model_refine)
            mpjpes.append(p1)
